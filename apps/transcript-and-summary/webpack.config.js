...
module.exports = {
  ...
  resolve: {
    ...
    alias: {
      ...
      // For webresource builds, redirect dynamic imports to stubs
      ...(isWebResource ? {
        '@shared/config/deploymentContext': path.resolve(__dirname, './src/stubs/deploymentContext-stub.ts'),
      } : {})
    },
    // For webresource builds, provide node polyfills
    fallback: isWebResource ? {
      "process": false, // Disable process polyfill, we'll use DefinePlugin instead
      "buffer": false,
    } : {},
  },
  module: {
  ...
  plugins: [
    new webpack.DefinePlugin({
      ...
      // For webresource builds, provide a minimal process polyfill
      ...(isWebResource ? {
        'process': `{
          env: {
            WEBPACK_DEPLOYMENT_MODE: ${JSON.stringify(deploymentMode)},
            NODE_ENV: ${JSON.stringify(isProduction ? 'production' : 'development')},
            API_BASE_URL: ${JSON.stringify(process.env.API_BASE_URL || 'http://localhost:3001/api')},
            REACT_APP_API_BASE_URL: ${JSON.stringify(process.env.REACT_APP_API_BASE_URL || '/api')},
            ENABLE_LOGGING: ${JSON.stringify(process.env.ENABLE_LOGGING || 'true')},
            ENABLE_OFFLINE: ${JSON.stringify(process.env.ENABLE_OFFLINE || 'false')},
            ENABLE_TELEMETRY: ${JSON.stringify(process.env.ENABLE_TELEMETRY || 'false')},
    ...
    ...(isWebResource ? [
      ...
      // Custom plugin to merge all chunks into main bundle
      {
        apply: (compiler) => {
          compiler.hooks.thisCompilation.tap('SingleChunkPlugin', (compilation) => {
            compilation.hooks.optimizeChunks.tap('SingleChunkPlugin', (chunks) => {
              const chunksArray = Array.from(chunks);
              const mainChunk = chunksArray.find(chunk => chunk.name === 'main');
              if (mainChunk) {
                // Merge all other chunks into main chunk (except CSS-only chunks)
                chunksArray.forEach(chunk => {
                  if (chunk !== mainChunk && !chunk.hasRuntime()) {
                    // Check if chunk has any JavaScript modules
                    const modules = compilation.chunkGraph.getChunkModules(chunk);
                    const hasJsModules = Array.from(modules).some(module =>
                      !module.type || module.type.includes('javascript')
                    );

                    if (hasJsModules) {
                      // Move all modules from this chunk to main chunk
                      modules.forEach(module => {
                        compilation.chunkGraph.disconnectChunkAndModule(chunk, module);
                        compilation.chunkGraph.connectChunkAndModule(mainChunk, module);
                      });
                      // Remove the chunk
                      chunks.delete(chunk);
                    }
  ...
  optimization: {
    ...
    ...(isWebResource ? {
      ...
      // Force all modules into main chunk
